...
- 已打开的页面标签同步资产颜色；独立圆点表示会话状态（绿色已连接、红色失败、黄色连接中、灰色空心已断开）
...
页面标签上沿色条和图标跟随资产的“颜色标签”，修改颜色后已打开的标签立即同步。旁边的状态圆点独立表示此页面的实际会话：绿色已连接，红色连接失败，黄色连接中，灰色空心已断开；悬停标签可查看完整标题及状态。同一资产打开多个页面时，状态分别跟随各自会话。SSH 页面中的嵌入式 SFTP 不覆盖 SSH 终端状态；单独打开的 SFTP / FTP 标签显示自己的文件连接状态。普通 SQL 语法错误、文件权限错误不会把仍然存活的连接误标为断线。
